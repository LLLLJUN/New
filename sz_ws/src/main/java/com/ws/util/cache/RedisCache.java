...
public class RedisCache {
    ...
    public <T> boolean putCache(String key, T value) {
        //调用统一方法
        return execute(key, value, 0,true);
    }
    
    /**
     * 插入一个普通对象，如String，实体类等
     * flag：true如果key已经存在，则覆盖这个key的value
     * 		false如果key已经存在，则添加缓存失败
     */
    public <T> boolean putCache(String key, T value,Boolean flag) {
        //调用统一方法
        return execute(key, value, 0,flag);
    }
    ...
    public <T> boolean putOverdueTime(String key, T value, final long overdueTime) {
        //调用统一方法
        return execute(key, value, overdueTime,true);
    }
    ...
    public <T> boolean putListCache(String key, List<T> value) {
        //调用统一方法
        return execute(key, value, 0,false);
     ...
     * @return boolean
     */
    private <T> boolean execute(final String key, final T value, final long overdueTime,final Boolean flag) {
